...
class LatticePlanner: 
    ...
    def plan_multiple(self, ego_state, starting_block, observation, traffic_light_data, top_k=5, diversity_threshold=1.0):
        edges = self.get_candidate_edges(starting_block, ego_state)
        candidate_paths = self.get_candidate_paths(edges)

        if candidate_paths is None: return None
        ...
        obstacles, vehicles = [], []
        for obj in objects:
            if obj.box.geometry.distance(ego_state.car_footprint.geometry) > 30: continue
            if obj.tracked_object_type == TrackedObjectType.VEHICLE:
                if obj.velocity.magnitude() < 0.01: obstacles.append(obj.box)
                else: vehicles.append(obj.box)
            else: obstacles.append(obj.box)

        paths = self.generate_paths2(ego_state, candidate_paths)

        if len(traffic_light_data) > 0: self._just_stay_current = True
        elif self.num_edges >= 4 and ego_state.dynamic_car_state.rear_axle_velocity_2d.x <= 3: self._just_stay_current = True
        else: self._just_stay_current = False

        scored_paths = []
        for path in paths:
            cost = self.calculate_cost(path, obstacles, vehicles)
            scored_paths.append((cost, path[0]))
        ...
        for cost, path_geom in scored_paths:
            if len(ref_paths) >= top_k:
                break
                
            is_diverse = True
            for sel_geom in selected_geometries:
                min_len = min(len(path_geom), len(sel_geom))
                if min_len > 10:
                    distances = np.linalg.norm(path_geom[:min_len, :2] - sel_geom[:min_len, :2], axis=1)
                    if np.max(distances) < diversity_threshold: 
                        is_diverse = False
                        break
                        
            if is_diverse:
                selected_geometries.append(path_geom)
                ref_path = self.post_process(path_geom, ego_state)
                ref_paths.append(ref_path)

        return ref_paths

    def generate_paths2(self, ego_state, paths):
        new_paths = []
        ego_state_coords = ego_state.rear_axle.x, ego_state.rear_axle.y, ego_state.rear_axle.heading
        
        lateral_offsets = [0.0, 1.2, -1.2]
        ...
        for _, (path_len, dist, path, path_polyline) in paths:
            ...
            for j, state in enumerate(target_states):
                for lat_offset in lateral_offsets:
                    target_x = state[0] - lat_offset * np.sin(state[2])
                    target_y = state[1] + lat_offset * np.cos(state[2])
                    
                    first_stage_path = calc_4points_bezier_path(ego_state_coords[0], ego_state_coords[1], ego_state_coords[2], 
                                                                target_x, target_y, state[2], 3, sampled_index[j])[0]
                    
                    second_stage_path = path_polyline[sampled_index[j]+1:, :2].copy()
                    if len(second_stage_path) > 0:
                        second_stage_headings = path_polyline[sampled_index[j]+1:, 2]
                        second_stage_path[:, 0] -= lat_offset * np.sin(second_stage_headings)
                        second_stage_path[:, 1] += lat_offset * np.cos(second_stage_headings)
                    
                    full_path_polyline = np.concatenate([first_stage_path, second_stage_path], axis=0)
                    new_paths.append((full_path_polyline, dist, path, path_len))     
