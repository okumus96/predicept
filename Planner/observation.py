...
def observation_adapter(history_buffer, traffic_light_data, map_api, route_roadblock_ids, device='cpu'):
    ...
    lanes = vector_map['map_lanes']
    data['lane_tl'] = (lanes[..., 3:7].clone() if torch.is_tensor(lanes) else torch.from_numpy(lanes[..., 3:7].copy()))
    polys = query_polygon_layers(map_api, ego_coords, [SemanticMapLayer.INTERSECTION, SemanticMapLayer.STOP_LINE], ego_state.rear_axle, radius)
    data['intersections'] = torch.from_numpy(polys[SemanticMapLayer.INTERSECTION])
    data['stop_polygons'] = torch.from_numpy(polys[SemanticMapLayer.STOP_LINE])
    data = convert_to_model_inputs(data, device)

    return data


def query_polygon_layers(map_api, point, layers, anchor, radius, max_elems=20, max_pts=20):
    import math
    import numpy as np
    outs = {layer: np.zeros((max_elems, max_pts, 3), dtype=np.float32) for layer in layers}
    try:
        got = map_api.get_proximal_map_objects(point, radius, layers)
    except Exception:
        return outs
    ...
    for layer in layers:
        ...
        for obj in got.get(layer, []):
            if k >= max_elems: break
            try:
                xy = np.array(obj.polygon.exterior.coords, dtype=np.float32)
            except Exception:
                continue
            if len(xy) < 2: continue
            if len(xy) > max_pts: xy = xy[np.linspace(0, len(xy) - 1, max_pts).astype(int)]
