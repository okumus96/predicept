...
from Predicept.train_utils import *
...
def train_epoch(data_loader, model, optimizer):
    ...
    with tqdm(data_loader, desc="Training", unit="batch") as data_epoch:
        for batch in data_epoch:
            # prepare data
            inputs = {
                'ego_agent_past': batch[0].to(args.device).float(),
                'neighbor_agents_past': batch[1].to(args.device).float(),
                'map_lanes': batch[2].to(args.device).float(),
                'map_crosswalks': batch[3].to(args.device).float(),
                'route_lanes': batch[4].to(args.device).float(),
                'c_lat_candidates': batch[7].to(args.device).float()
            }
            ...
            gt_mode_idx = get_expert_mode_index(ego_future, c_lat_candidates)
            # call the mdoel
            optimizer.zero_grad()
            level_k_outputs, ego_plan, neural_plan = model(inputs)
            loss, results = level_k_loss(level_k_outputs, ego_future, neighbors_future, neighbors_future_valid)
            prediction = results[:, 1:]
            plan_loss = planning_loss(neural_plan, ego_future)
            ...
            # compute metrics
            metrics = motion_metrics(neural_plan, prediction, ego_future, neighbors_future, neighbors_future_valid)
    ...
    with tqdm(data_loader, desc="Validation", unit="batch") as data_epoch:
        for batch in data_epoch:
           # prepare data
            ...
            gt_mode_idx = get_expert_mode_index(ego_future, inputs['c_lat_candidates'])
